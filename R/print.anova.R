...
#'  
#' Modifies the print.anova()) function so that the output is less verbose (the heading and several \sQuote{extra} blank lines were removed) and, if a single anova object is sent, then a \sQuote{total} row will be added to the anova table.
#' 
#' @aliases print.anova
#' 
...
#' @param rm.heading A logical indicating whether the heading in the anova object should be included or not
...
print.anova <- function(x,digits=max(getOption("digits")-2,3),signif.stars=getOption("show.signif.stars"),
  ...
  # exclusion for when multiple lm objects are sent
  if (!any(grepl("Res.Df",colnames(x)))) {
    # exclusion for levenes.test
    if (!any(grepl("Levene",attr(x,"heading")))) {
